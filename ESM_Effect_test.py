...
class ExperimentManager:
    ...
    def evaluate(self, data_loader: DataLoader, prefix: str = "val") -> Dict[str, float]:
        ...
        fpr, tpr, _ = roc_curve(all_classifications[all_classifications != 0], 
                         all_preds[all_classifications != 0])
        auroc = roc_auc_score(all_classifications[all_classifications != 0], 
                            all_preds[all_classifications != 0])
        ...
        # 이미지 저장
        plt.savefig('clinvar_roc.jpg', dpi=200, bbox_inches='tight')
        plt.close()
...
def plot_correlation(df, x_col, y_col, title, figsize=(8, 8)):
    ...
    plt.tight_layout()
    plt.savefig('ESM_experiment_clinvar_test.jpg')
    plt.show()
    plt.close(fig)

# Load the DataFrame
test = pd.read_csv("data/clinvar_test.csv")
...
model = ESMEffectFull()
experiment = ExperimentManager(model, config, "protein_prediction_20250225_132437") # protein_prediction_20250225_113711
...
test_dataset = ProteinDatasetESMEffect(test, config['feature_columns'])
...
print(experiment.evaluate(test_loader))
